...
import com.lss.model.DormBuild;
import com.lss.model.DormManager;
import com.lss.model.PageBean;
import com.lss.util.DbUtil;
import com.lss.util.PropertiesUtil;
...
public class DormBuildServlet extends HttpServlet{
	...
	private void dormBuildDelete(HttpServletRequest request,
		...
		try {
			...
			if(dormBuildDao.existManOrDormWithId(con, dormBuildId)) {
				request.setAttribute("error", "错误0001");
			} else {
	...
	private void dormBuildSave(HttpServletRequest request,
		...
		try {
			...
			if(saveNum > 0) {
				...
				request.setAttribute("error", "错误01");
	...
	private String genPagation(int totalNum, int currentPage, int pageSize){
		...
		pageCode.append("<li><a href='dormBuild?page=1'>{</a></li>");
		if(currentPage==1) {
		...
		pageCode.append("<li><a href='dormBuild?page="+totalPage+"'>}</a></li>");
